...
export class PharmacyOrderService {
    ...
    async listShopOrders(query: ListOrdersQuery) {
        ...
        const qb = this.dataSource
                ...
                // لود کردن جزئیات محصولات داخل هر آیتم سفارش
                .leftJoinAndSelect('items.marketProduct', 'marketProduct')
                .leftJoinAndSelect('marketProduct.product', 'product')
                .leftJoinAndSelect('items.pharmacyMedicine', 'pharmacyMedicine')
                .leftJoinAndSelect('pharmacyMedicine.medicine', 'medicine')
                .leftJoinAndSelect('items.variant', 'variant')//TODO:marketProduct=>product
                .where('order.tenantId = :tenantId', {tenantId})
            //.andWhere('order.type = :type', {type: OrderType.PRODUCT})
        ;
        if (query.status) {
    ...
    async rejectOrder(orderId: string, reason?: string) {
        const tenantId = this.tenantContext.getTenantId();
        // برای پیدا کردن userId از روی سفارش نیاز به یک کوئری اضافی داریم یا می‌توانیم از order موجود استفاده کنیم
        const order = await this.dataSource.getRepository(Order).findOne({
            where: {id: orderId, tenantId, type: OrderType.PRODUCT},
            select: ['userId']
        } as any);
        ...
        // فراخوانی متد لغو با پرچم اینکه این عمل توسط تنت انجام شده است
        return this.cancelOrder(orderId, order.userId, true);
    }
     ...
     * It also handles side effects like refunds and inventory restoration for cancellation states.
     *
    ...
    async updateOrderStatus(orderId: string, newStatus: OrderStatus) {
        return this.dataSource.transaction(async (manager: EntityManager) => {
            ...
            // 2️⃣ Fetch the order to ensure it exists
            const order = await manager.findOne(Order, {
                where: {
                    id: orderId,
                    tenantId,
            ...
            // 3️⃣ Check if we are transitioning to a Cancellation State
            // If so, we must handle refunds and inventory BEFORE or DURING the state change.
            // Note: cancelOrder and rejectOrder are designed to handle their own transactions,
            // but since we are inside a transaction here, we need to be careful.
            // However, based on the previous design, cancelOrder expects to manage the whole flow.
            // To avoid nested transaction conflicts, we can call them outside or ensure they use the current manager if supported.
            // In NestJS/TypeORM, it's safer to let the specific service method handle its transaction if it's complex.
            // BUT, to keep it atomic with the status update in this context, let's try to integrate.

            // Alternative Approach: Since cancelOrder already handles the status update via FSM internally in the previous code,
            // calling updateOrderStatus -> cancelOrder might cause double FSM calls.
            // Let's refactor slightly: If the target is cancellation, let the specific method handle it entirely.

            if (newStatus === OrderStatus.CUSTOMER_CANCELLED) {
                // User initiated cancellation
                // We need userId. Since we are in tenant context, we assume the caller passes userId, or we fetch it.
                // For this generic method, let's assume we fetch the user from the order relation if loaded,
                // or we rely on the fact that this method might be called by UserController which has userId.
                // Since userId is not passed to updateOrderStatus, we must fetch it from order.
                const userId = order.userId;

                // Call the specific cancellation logic.
                // Note: This will open a NEW transaction inside this one if cancelOrder uses transaction().
                // To prevent issues, cancelOrder should ideally accept an existing manager.
                // Assuming cancelOrder is robust enough or we catch errors.
                // For this implementation, I will inline the logic or call the method.
                // Given the complexity, calling the service method is cleaner.

                try {
                    // We need to pass userId. We got it from order.
                    await this.cancelOrder(orderId, userId, false);

                    // If successful, the status is already updated by cancelOrder via FSM.
                    // We just return the order.
                    ...
                    throw error; // Re-throw if refund/inventory fails
                }
            }
            ...
            // 4️⃣ For non-cancellation statuses, proceed with standard State Machine transition
            try {
    ...
    // در فایل استور سرویس یا کنترلر مربوط به سفارشات

    async updateMetadataOrder(orderId: string, updateMetadata: any) {
        return this.dataSource.transaction(async (manager: EntityManager) => {
            // 1️⃣ Retrieve Tenant ID
            const tenantId = this.tenantContext.getTenantId();

            // 2️⃣ Fetch the order to ensure it exists and get current data
            const order = await manager.findOne(Order, {
                where: {
                    id: orderId,
                    tenantId,
            ...
            // 3️⃣ Merge new metadata with existing metadata to avoid overwriting other fields
            const existingMetadata = order.metadata || {};
            const mergedMetadata = { ...existingMetadata, ...updateMetadata };


            try {
                // 4️⃣ Update the metadata column
                ...
                // 5️⃣ Fetch the updated order to return complete data
                const updatedOrder = await manager.findOne(Order, {
                ...
                return {
                    orderId: updatedOrder?.id,
                    status: updatedOrder?.status,
                    metadata: updatedOrder?.metadata // بازگرداندن متادیتای جدید
                };

            } catch (error) {
                throw new BadRequestException(`Failed to update order metadata: ${error.message}`);
            }
        });
    }
}